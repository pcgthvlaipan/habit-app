            ...
            <>
              ...
              {forgotSent ? (
                <div style={{ textAlign: "center", padding: "8px 0 16px" }}>
                  ...
                  >
                    Click the link in the email to set a new password. Check
                    your spam folder if you don't see it.
                  </p>
                  <button className="submit-btn" onClick={goToSignIn}>
                    ← Back to sign in
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <button className="cancel-btn" onClick={goToSignIn}>
                    ← Back to sign in
                  </button>
                </>
              )}
...
function Dashboard({ authUser }) {
      ...
      subscribeToHabits(
        ...
        (fresh) => {
          ...
          if (
            ...
            Date.now() - pendingLogRef.current < 3000
          ) {
            ...
            setHabits((prev) =>
              fresh.map((fh) => {
                ...
                if (ph && ph._rawLogs?.length > fh._rawLogs?.length) {
                  return {
                    ...fh,
                    _rawLogs: ph._rawLogs,
                    todayStatus: ph.todayStatus,
                    todayPartial: ph.todayPartial,
                  };
                }
                return fh;
              })
  ...
  function handleLog(hid, status, partial) {
    ...
    const todayKey = `${bkk.getUTCFullYear()}-${String(
      bkk.getUTCMonth() + 1
    ).padStart(2, "0")}-${String(bkk.getUTCDate()).padStart(2, "0")}`;
      ...
      prev.map((h) => {
        ...
        let newRawLogs = (h._rawLogs ?? []).filter((l) => {
          ...
          const ld = l.date?.toDate
            ? l.date.toDate()
            : new Date(l.date?.seconds * 1000 ?? l.date);
          const lBkk = new Date(ld.getTime() + 7 * 60 * 60 * 1000);
          const lKey = `${lBkk.getUTCFullYear()}-${String(
            lBkk.getUTCMonth() + 1
          ).padStart(2, "0")}-${String(lBkk.getUTCDate()).padStart(2, "0")}`;
      ...
      <div className="phone">
        ...
        {tab === "today" && (
          <>
            {notifPerm !== "granted" && (
              <NotifBanner
                onAllow={handleRequestNotifPermission}
...
function HabitCard({ habit, selected, onSelect, onLog, onEdit, onDelete }) {
  ...
  const partialText =
      ...
      ? `${todayPartial.value}${
          todayPartial.unit ? " " + todayPartial.unit : ""
        } / ${todayPartial.target}${
          todayPartial.unit ? " " + todayPartial.unit : ""
        }`
      : null;
        ...
        <div className="habit-right">
          {notSched ? (
            ...
            <div className="log-btns">
              ...
              <button
                className={`log-btn log-miss${
                  missed ? " log-miss--active" : ""
                }`}
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="modal-title">
          Log Progress — {habit.icon} {habit.name}
        </p>
        ...
        <label className="field-label">
          ...
          <span style={{ float: "right", color: "var(--text-2)" }}>
            Target: {target}
            {unit ? ` ${unit}` : ""}
          </span>
        </label>
        ...
        {value > 0 && value < target && (
          ...
          >
            {pct >= 75
              ? "💪 Almost there! Every bit counts."
              : pct >= 50
              ? "⚡ Halfway done is still progress!"
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <label className="field-label" style={{ marginTop: 16 }}>
          ...
          >
            Optional — enables partial logging
          </span>
        </label>
          ...
          {reminderOn && (
            <div className="reminder-time-wrap">
              ...
              <p className="reminder-hint">
                📱 In-app + 📅 Google Calendar reminders will be set
              </p>
            </div>
          )}
        </div>
...
function CalendarView({ habit, uid }) {
  ...
  function bangkokDateStr(date = new Date()) {
    ...
    return `${bkk.getUTCFullYear()}-${pad(bkk.getUTCMonth() + 1)}-${pad(
      bkk.getUTCDate()
    )}`;
  }
  ...
  const partialMap = useMemo(() => {
    ...
    habit._rawLogs.forEach((log) => {
      ...
      const key = `${bkk.getUTCFullYear()}-${p(bkk.getUTCMonth() + 1)}-${p(
        bkk.getUTCDate()
      )}`;
  ...
  const calDays = useMemo(() => {
    ...
    return days.map((d) => ({
      ...
      partial:
        localPartial[d.date] !== undefined
          ? localPartial[d.date]
          : partialMap[d.date] ?? null,
    }));
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        <span
          ...
          style={isPartial ? { color: "#92400E" } : {}}
        >
          {d.day}
        </span>
        {d.status === "done" && !isPartial && (
          <span className="cal-day-icon">✓</span>
        )}
        {d.status === "missed" && <span className="cal-day-icon">✗</span>}
        {isPartial && (
          <span
            className="cal-day-icon"
      ...
      {open && !isFuture && (
        <div className="day-log-menu" onClick={(e) => e.stopPropagation()}>
          ...
          {isPartial && (
            ...
            >
              ◑ {partial.value}
              {partial.unit ? " " + partial.unit : ""} / {partial.target}
              {partial.unit ? " " + partial.unit : ""} ({partial.pct}%)
            </p>
          )}
          <div style={{ display: "flex", gap: 5 }}>
            <button
              className={`day-log-btn day-log-done${
                d.status === "done" && !isPartial ? " active" : ""
              }`}
              onClick={() => tap("done")}
            >
              ✓ Done
  ...
  return (
    <div className="chart-card anim-4">
      ...
      <div className="bar-chart">
        {chartData.map((d, i) => {
          const isPartial =
            d.status === "done" && d.partial && d.partial.pct < 100;
          const isFullDone = d.status === "done" && !isPartial;
          const barHeight = isFullDone
            ? "100%"
            : isPartial
            ? `${d.partial.pct}%`
            : "0%";
          ...
          return (
            <div
              key={i}
              className="bar-col"
              title={isPartial ? `${d.partial.pct}% done` : d.status}
            >
...
function HabitStatCard({ habit }) {
  const { name, icon, color, streak, successRate, totalDone, totalLogged } =
    habit;
  ...
  return (
    <div className="stat-detail-card">
        ...
        {chartData30d.map((d, i) => {
          ...
          return (
            <div
              ...
              title={
                isFullDone ? "Done" : isPartial ? `${d.partial.pct}% done` : ""
              }
            >
              <div
                style={{
      ...
      >
        ...
        />
        <span style={{ fontSize: 10, color: "var(--text-2)" }}>Partial</span>
        <div
          style={{
            width: 10,
            height: 10,
        ...
        />
        <span style={{ fontSize: 10, color: "var(--text-2)" }}>Missed</span>
      </div>
    </div>
  );
}
...
function NotifBanner({ onAllow, denied }) {
  ...
  return (
    <div className="notif-banner">
      ...
      {!denied && (
        <button className="notif-allow-btn" onClick={onAllow}>
          Allow
        </button>
      )}
      <button className="notif-dismiss-btn" onClick={() => setDismissed(true)}>
        ✕
      </button>
    </div>
  );
...
function RemindersTab({
  ...
  return (
    <div style={{ padding: "0 16px" }}>
      <div className="reminder-status-card">
        ...
        >
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--text-2)", marginTop: 2 }}>
              {notifPerm === "granted"
                ? "You will receive in-app alerts for enabled habits"
                : notifPerm === "denied"
                ? "Open browser settings → Notifications → Allow this site"
                : "Tap below to enable habit reminders"}
            </p>
          </div>
        </div>
      ...
      <p
        className="section-label"
        style={{ padding: 0, marginTop: 20, marginBottom: 12 }}
      >
        Habit Reminders
      </p>
      {habits.length === 0 ? (
        <div className="habit-empty">
          ...
          <p className="habit-empty-sub">
            Add a habit first, then set reminders here.
          </p>
        </div>
      ) : (
        habits.map((h) => (
      ...
      >
        ...
        >
          Google Calendar reminders work even when your phone is off. Enable a
          reminder on any habit above, then tap "Add to Google Calendar" to
          create a recurring daily event with alerts.
        </p>
        <p style={{ fontSize: 11, color: "var(--text-3)" }}>
          Reminders are set to your Bangkok timezone (UTC+7)
        </p>
      </div>
    </div>
  );
...
function ReminderCard({
  ...
  return (
    <div className="reminder-card">
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <p
            style={{
              fontSize: 12,
              color: hasReminder ? "var(--green)" : "var(--text-3)",
              marginTop: 2,
              fontWeight: 600,
            }}
          >
        ...
        <button className="reminder-edit-btn" onClick={onEdit}>
          ✏️ Edit
        </button>
      </div>
      {hasReminder && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {notifPerm === "granted" && (
            <button
              className="reminder-action-btn notif-test-btn"
              onClick={onTest}
            >
              🔔 Test alert
            </button>
          ...
          <button
            className={`reminder-action-btn gcal-btn${
              gcalStatus === "creating" ? " loading" : ""
            }`}
      ...
      {!hasReminder && (
        <button className="reminder-set-btn" onClick={onEdit}>
          + Set reminder time
        </button>
      )}
    </div>
